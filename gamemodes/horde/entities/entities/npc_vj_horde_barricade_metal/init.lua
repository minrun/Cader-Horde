...
function ENT:CustomOnMeleeAttack_AfterChecks( hitEnt, isProp )
	...
	if self.Horde_Debuff_Active and self.Horde_Debuff_Active[HORDE.Status_Shock] then
		hitEnt:Horde_AddDebuffBuildup( HORDE.Status_Shock, 10, self )
...
function ENT:CustomOnTakeDamage_BeforeDamage( dmginfo, hitgroup )
	...
	if dmginfo:GetDamage() >= math.floor(self:Health() - 100)
	then 
		dmginfo:ScaleDamage( 1 )
		dmginfo:SetDamage( math.floor( self:Health() - 100 ) )
	end
end
